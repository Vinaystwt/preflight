...
const paid = await paidFetch(endpoint, { method: "POST", headers: { "content-type": "application/json", "idempotency-key": `${idempotency}-paid` }, body: JSON.stringify(request) }); const report = await paid.json() as { report_id?: string; decision?: string; report_access?: { access_token?: string; report_url?: string } };
gates.push({ gate: "paid report", result: paid.ok && Boolean(report.report_id && report.decision) ? "PASS" : "FAIL", detail: `HTTP ${paid.status}; decision=${report.decision ?? "missing"}` }); gates.push({ gate: "settlement receipt", result: Boolean(paid.headers.get("payment-response")) ? "PASS" : "FAIL", detail: paid.headers.get("payment-response") ? "present" : "missing" });
