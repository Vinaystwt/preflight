...
describe("public routes", () => {
  ...
  it("scopes CORS to approved origins and public routes", async () => {
    ...
    const www = await app.inject({ method: "GET", url: "/health", headers: { origin: "https://www.usepreflight.xyz" } });
    expect(www.headers["access-control-allow-origin"]).toBe("https://www.usepreflight.xyz");
    const fallback = await app.inject({ method: "GET", url: "/health", headers: { origin: "https://preflight-web-bice.vercel.app" } });
    expect(fallback.headers["access-control-allow-origin"]).toBe("https://preflight-web-bice.vercel.app");
    ...
    const lookalikeApex = await app.inject({ method: "GET", url: "/health", headers: { origin: "https://usepreflight.xyz.attacker.example" } });
    expect(lookalikeApex.headers).not.toHaveProperty("access-control-allow-origin");
    const lookalikeHyphen = await app.inject({ method: "GET", url: "/health", headers: { origin: "https://evil-usepreflight.xyz" } });
    expect(lookalikeHyphen.headers).not.toHaveProperty("access-control-allow-origin");
    const lookalikeFallback = await app.inject({ method: "GET", url: "/health", headers: { origin: "https://preflight-web-bice.vercel.app.attacker.example" } });
    expect(lookalikeFallback.headers).not.toHaveProperty("access-control-allow-origin");
